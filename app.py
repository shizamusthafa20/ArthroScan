...
async def predict(file: UploadFile = File(...)):
    ...
    # Calculate Activation Coverage (Infestation Level)
    try:
        # Coverage is the % of pixels with > 0.4 activation
        act_threshold = 0.4
        coverage = np.mean(grayscale_cam > act_threshold)
    ...
    # Get DB info
    db_info = copy.deepcopy(pest_db.get(predicted_class, pest_db["Default"]))
    db_info["impact"]["infestation_level"] = f"{inf_level} Infestation"
